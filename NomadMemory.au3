...
Func _MemoryScanEx($ah_Handle, $pattern, $mask , $after = False, $iv_addrStart = 0x00400000, $iv_addrEnd = 0x00FFFFFF, $step = 100)
    If Not IsArray($ah_Handle) Then
        SetError(1)
        Return -1
    EndIf
...
EndFunc   ;==>_MemoryScan
;~ EndFunc   ;==>_MemoryScan
Func _MemoryScanEx_special($ah_Handle, $pattern, $mask , $after = False, $iv_addrStart = 0x00400000, $iv_addrEnd = 0x00FFFFFF, $step = 100000)
    Local $formatedpattern=""
    Local $BufferPattern
    Local $BufferMask
;~     for $i = 0 to stringlen($mask)-1
...
;~         $formatedpattern = $formatedpattern&$BufferPattern
;~     Next
    $pattern = BinaryToString("0x"&$pattern)
    For $addr = $iv_addrStart To $iv_addrEnd Step $step - ((StringLen($pattern)-6) / 2)
        StringRegExp(BinaryToString(_MemoryRead($addr, $ah_Handle, "byte[" & $step & "]")), "(?i)"&$pattern, 1)
        If Not @error Then
			Return StringFormat("0x%.8X", $addr + ((@extended - StringLen($pattern)+6)-1))
		EndIf
    Next
    Return -3
EndFunc
